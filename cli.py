...
def main():
    ...
    parser.add_argument("--input", required=False, help="Input MP4 file")
    ...
    if not args.input:
        args.input = input("Enter input MP4 file path: ").strip()
        if not args.input:
    ...
    tags = ranker.rank_and_filter(captions, candidates, top_k=args.top)
